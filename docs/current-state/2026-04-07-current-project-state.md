...
- 规则说明可读性优化：统计页（`mirror-table-overview`、`system-test-defect-summary`）已开始改造成面向 QA 的“先看结论 / 统计过程 / 这个数字怎么算”展示；`code-review-illegal-records` 仍保留原一期结构，尚在收尾
...
- 统计页的规则说明抽屉已开始做 QA 友好化改造：
  - 已完成：统计页默认先展示“结论、数量变化、保留比例、步骤解释”
  - 未完成：代码走查非法记录页仍未完全切换到同一套 QA 友好展示
- 本地镜像驱动的记录页已存在一张：
  - 代码走查非法记录
- 独立采集表单页已存在：
  - 代码走查表
