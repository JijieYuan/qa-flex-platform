...
- 规则说明可读性优化：统计页（`mirror-table-overview`、`system-test-defect-summary`）已改造成面向 QA 的“先看结论 / 哪些会被排除 / 数据如何一步步变少 / 最后这些数字怎么算”卡片式展示；`code-review-illegal-records` 仍保留原一期结构，尚在收尾
