...
- 后端列模型已支持 `children + columns` 的递归树结构
- 前端 `BaseStatisticTable` 已支持递归渲染多级分组表头
- 列显示设置、排序列识别、导出列展开都已切到“按叶子列处理”
- 该能力不是仅供某一张统计表使用，而是已经沉入统计表抽象层
- 2026-04-09 补充：已修正“顶层分组只渲染 children、不渲染顶层直接叶子列”的抽象层缺口；现在像二级缺陷、三级缺陷、建议类、新发议题、遗留率这类直接挂在顶层分组上的列也会正常渲染
- 2026-04-09 再补充：已修正“旧的列顺序偏好没有自动合并新增列”问题；现在即使用户本地已有旧视图配置，新增加的列只要勾选，也会被自动补进实际渲染顺序
- 2026-04-09 再补充：已修正“嵌套小表头拖拽排序虽然写入偏好，但递归列树重建时又被原始顺序覆盖”的问题；当前递归组装过程已按用户拖拽后的叶子列顺序回填
- 2026-04-09 再补充：已移除统计列标题旁统一渲染的 `?` 计算提示，避免把表头排版挤乱；后续若需要解释口径，优先放到规则说明抽屉而不是塞在每一列表头里
...
- 数据来源已切为 `issue_fact` 优先，必要时再回退 ODS
- 统计规则的核心判定已沉入 `FactBuildService + IssueFact*Rules`
- 支持统计表、详情下钻、导出、最近同步展示
- 支持“规则说明”只读抽屉，展示内置规则版本、过滤 Flow 步骤和指标口径说明
- 当前页面“刷新”行为是重新拉取当前统计结果，不是页面级实时刷新壳
- 当前行维度为 `模块名称`
- 当前表头已按老平台 `ModuleTable` 方向切为模块维度多级表头

当前过滤步骤（代码事实）：

- 初始范围：当前查询条件下从 `issue_fact` 读取的议题
- 范围收口：仅保留带有“系统测试 / 回归测试”标签的议题
- 排除携带“功能屏蔽”“已拒绝”“建议”标签的议题
- 排除关闭状态下携带“申请否决”或“需求如此”标签的议题
- 模块展开：同一议题可按多个模块展开到多行聚合

当前规则说明能力（代码事实）：

- 当前内置规则版本号：`system-test-defect-summary@2026-04-09-v5`
- 规则说明接口返回的是**当前代码真实执行口径**，不是独立配置中心中的外部可编辑规则
- Flow 步骤展示的是“加载事实议题 -> 限定系统测试范围 -> 排除无效议题 -> 模块展开”的顺序计数过程

当前分类方式（代码事实）：

- 一级 / 二级 / 三级 / 建议类：读取 `issue_fact.severity_level`
- P1 / P2 / P3：读取 `issue_fact.priority_level`
- 一级中的“回退 / 挂机 / 其他”：读取 `issue_fact.is_regression / is_crash / is_level1_other`
- 已修复 / 遗留 / 延期 / 非法等状态：读取事实字段，不在 BoardService 内重复判定
...
- 表头结构已对齐老平台方向，但个别列的业务口径仍是基于当前 `issue_fact` 字段做的第一版映射
- `已修复/未更新` 当前在新平台中暂以 `solved_count` 对齐“已修复数量”语义，不代表已经完全复刻老平台历史实现细节
...
- 已继续修正系统测试规则落地：
  - 系统测试缺陷汇总只统计带有“系统测试 / 回归测试”范围标签的议题
  - 规则说明中已新增“限定系统测试范围”步骤
  - 公共过滤里的 `设计如此+Closed` 已纠正为 `需求如此+Closed`
  - 系统测试非法数据已补：
    - `未按照模板回复`
    - `缺陷原因不唯一`
    - 多条原因评论按最后一条评论解析
  - 系统测试看板中分母为 `0` 的比率展示已改为 `/`
...
### 2026-04-09 两张已实现统计表的补漏修复

- 本轮先按“先收口现有两张表功能缺口，再谈规则编辑”的顺序继续修复：
  - `system-test-defect-summary`
  - `code-review-illegal-records`

#### 系统测试缺陷汇总

- 已修正综合区 `已修复/未更新` 与 `修复率` 的统计口径：
  - 当前按照 `已修复标签命中 或 议题已关闭` 视为已解决
  - 不再只按 `is_fixed = true` 单独统计
- 已修正 `新发议题` 的修复数量口径：
  - 新发议题仍按“排除历史遗留”的口径统计
  - 新发议题修复数量改为按“已修复标签命中或议题已关闭”统计
- 已修正 `新发议题关闭率` 的统计口径：
  - 当前按“新发议题 + 已关闭且已进入修复态”统计
  - 不再把任意关闭状态都直接算进新发关闭数量
- 已修正 `遗留率` 相关口径：
  - `一级缺陷遗留率`
  - `二级缺陷遗留数量`
  - `三级缺陷遗留数量`
  - `二三级缺陷遗留率`
  - 以上均改为基于 `issue_fact.is_legacy` 统计
  - 不再把“未关闭”直接当作“历史遗留”
- 已同步更新规则说明文案：
  - 综合汇总明确说明“已修复/未更新”的计算口径
  - 新发议题明确说明为“排除历史遗留”
  - 遗留率明确说明为 `issue_fact.is_legacy` 口径

#### 代码走查非法记录

- 规则说明入口已改为始终可见：
  - 即使规则说明接口失败，按钮也不会消失
  - 页面会展示降级说明，而不是看起来像没开发
- 非法筛选项来源已修正：
  - 现在只从“实际非法记录集合”中生成下拉选项
  - 不再出现能选但筛不出结果的选项
- 关键字搜索范围已扩大：
  - 已补 `模块`
  - `目标分支`
...
- 非法规则已继续补充：
  - `无代码走查`
  - `未代码扫描`（仅在事实层明确给出未扫描状态时命中）
  - `静态扫描问题未关闭`（仅在扫描问题数大于 0 时命中）
- 当前仍然保守处理扫描规则：
  - 如果事实层没有扫描状态，不会因为“未知”直接判成非法
  - 这是为了避免在外部扫描来源尚未完全接通前出现整表误判

#### 当前剩余边界

- `system-test-defect-summary` 仍然没有补成完整的页面筛选字段体系，当前主要依赖页面公共筛选和既有范围口径
- `code-review-illegal-records` 仍未覆盖所有老平台口径中的异常类型，例如：
  - 合法评论异常
  - 标题异常
  - 更细的走查时长异常
