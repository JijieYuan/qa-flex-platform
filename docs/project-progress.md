...
- 集成测试模块当前已有真实链路，且已完成第一轮解析容错与导出能力补强；仍需继续补：
  - 更完整的校验口径
  - 更多真实备注样例覆盖
  - 更完整的验证覆盖
...
- 集成测试数据分析第一轮补强
  ...
  - 新增 `IntegrationTestNoteParser`，支持 `key: value` 与 markdown 表格行两类备注格式。
  - 补充 `IntegrationTestControllerTest`、`IntegrationTestNoteParserTest` 和前端导出冒烟覆盖。
...
5. 继续补强 `集成测试数据分析` 的真实样例覆盖和更完整校验口径。
