...
- [ ] 定义 Source Adapter 接口边界。
- [ ] 定义表级同步状态机。
- [ ] 定义 Webhook 事件状态机。
- [ ] 定义事实刷新任务状态机。
- [ ] 定义多源隔离规则：所有入口如何获得 `configId`。

验收标准：

- [ ] 能画出 direct/docker 到 mirror/fact/diagnostics 的完整链路。
- [ ] 每个状态机都有成功、失败、重试、取消或降级路径。
- [ ] 能明确哪些现有表复用，哪些表需要新增或扩展。

验证：

- [ ] 方案评审通过。
- [ ] Flyway 迁移草案通过人工 review。
...
- [ ] 将 direct 和 docker 查询能力收敛到统一 Source Adapter。
- [ ] direct 元数据发现统一使用 `pg_catalog`。
- [ ] 补充只读权限诊断：连接、schema 读取、主键发现、白名单发现。
- [ ] 增加源表 schema fingerprint。
- [ ] 标记无更新时间字段表的同步策略。
- [ ] direct 模式明确输出 Webhook 需要手动注册。

验收标准：

- [ ] 只读账号可以发现表、主键和更新时间字段。
- [ ] direct 诊断可以区分连接失败、权限不足、主键缺失、白名单为空、schema 变化。
- [ ] direct 模式不会尝试执行 Docker/GitLab Rails Webhook 注册命令。
- [ ] Docker 模式旧行为不回退。

验证：

- [ ] `GitlabExternalDbServiceTest`
- [ ] direct 只读账号元数据发现测试
- [ ] 本地 Docker GitLab 手动联调脚本
...
- [ ] 拆分 `sourceEnabled`、`autoSyncEnabled`、`webhookEnabled` 的语义。
- [ ] 保存配置时校验启用 Webhook 的源必须有唯一非空 secret。
- [ ] Webhook 入口只路由到唯一匹配且 `webhookEnabled=true` 的配置。
- [ ] direct 配置页展示 receiver URL、secret 状态和手动注册提示。
- [ ] 健康诊断展示 secret 缺失、重复、禁用和模式不支持自动注册等状态。

验收标准：

- [ ] 关闭自动定时同步不影响 Webhook 接收。
- [ ] 多个启用 Webhook 的源不能使用相同 secret。
- [ ] 无 secret 或重复 secret 的配置在上线前诊断中可见。
- [ ] direct 模式用户不会误以为平台会自动注册 Webhook。

验证：

- [ ] `GitlabConfigServiceTest`
- [ ] `GitlabWebhookServiceTest`
