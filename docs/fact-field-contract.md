...
| --- | --- | --- | --- | --- | --- |
| `issue_fact` | `issue_category` | 根据标签、标题、备注和范围规则生成议题分类 | 按分类过滤和聚合 | 客户问题、系统测试分类展示 | 规则变化需要补充 rule 层测试 |
...
| `issue_fact` | `sla_*` | 根据计划解决时间和响应数据计算 SLA 口径 | 过滤、排序和统计 | 响应效率类页面 | 修改规则前先确认产品口径 |
| `integration_test_fact` | 用例统计字段 | 从备注解析结果生成执行、通过、未通过等计数 | 汇总、明细查询和导出 | 集成测试分析页 | 解析规则变化需要补充端到端链路测试 |
